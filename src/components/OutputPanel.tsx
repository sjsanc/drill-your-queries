...
function ResultTable({
    ...
    if (result.rows.length === 0) {
        return (
            <p className="text-xs text-zinc-400 p-3">Query returned no rows.</p>
        );
    }
                ...
                <thead className="sticky top-0 bg-zinc-50 z-10">
                    <tr>
                        {result.columns.map((col) => {
                            const bad =
                                comparison &&
                                comparison.columnMatch[col] === false;
                            return (
                                <th
                                    key={col}
...
export default function OutputPanel({
    ...
    const comparison =
        result && expectedResult && passed !== null
        ...
        <Group orientation="vertical" className="h-full">
            ...
            >
                ...
                <div className="flex-1 overflow-auto dot-grid">
                    ...
                    {result && !error && (
                        <ResultTable result={result} comparison={comparison} />
                    )}
                    {!result && !error && (
